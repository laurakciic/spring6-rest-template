...
import com.laurakovacic.spring6resttemplate.config.OAuthClientInterceptor;
import com.laurakovacic.spring6resttemplate.config.RestTemplateBuilderConfig;
...
public class BeerClientMockTest {
    ...
    void deleteNotFound() {
        server.expect(method(HttpMethod.DELETE))
                .andExpect(header("Authorization", BEARER_TEST)).andExpect(header("Authorization", BEARER_TEST))
                .andExpect(requestToUriTemplate(URL + BeerClientImpl.GET_BEER_BY_ID_PATH, dto.getId()))
    ...
    private void mockGetOperation() {
        server.expect(method(HttpMethod.GET))   // server mock for a GET method
                .andExpect(header("Authorization", BEARER_TEST))
                .andExpect(requestToUriTemplate(URL + BeerClientImpl.GET_BEER_BY_ID_PATH, dto.getId())) // binding method to path
                .andRespond(withSuccess(dtoJson, MediaType.APPLICATION_JSON)); // expecting JSON response object
    }
